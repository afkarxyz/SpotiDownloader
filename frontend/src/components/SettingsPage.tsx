...
import { FolderOpen, Save, RotateCcw, Info, MonitorCog, FolderCog, FolderLock, Router } from "lucide-react";
...
import { FlacIcon, Mp3Icon } from "./FormatIcons";
                ...
                setTempSettings(prev => ({
                    ...
                    folderTemplate: value === "custom" ? (prev.folderTemplate || preset.template) : preset.template
                      ...
                      {tempSettings.folderTemplate && (<p className="text-xs text-muted-foreground">
                        Preview: <span className="font-mono">{tempSettings.folderTemplate.replace(/\{artist\}/g, tempSettings.separator === "comma" ? "Kendrick Lamar, SZA" : "Kendrick Lamar; SZA").replace(/\{album\}/g, "Black Panther").replace(/\{album_artist\}/g, "Kendrick Lamar").replace(/\{year\}/g, "2018").replace(/\{date\}/g, "2018-02-09").replace(/\{isrc\}/g, "USUM71801234")}/</span>
                      </p>)}
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <Switch id="playlist-owner-folder-name" checked={tempSettings.playlistOwnerFolderName} onCheckedChange={(checked) => setTempSettings(prev => ({ ...prev, playlistOwnerFolderName: checked }))}/>
                       <Label htmlFor="playlist-owner-folder-name" className="text-sm cursor-pointer font-normal">Playlist Owner Folder Name</Label>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      <Switch id="redownload-with-suffix" checked={tempSettings.redownloadWithSuffix} onCheckedChange={(checked) => setTempSettings(prev => ({ ...prev, redownloadWithSuffix: checked }))}/>
                       <Label htmlFor="redownload-with-suffix" className="text-sm cursor-pointer font-normal">Redownload With Suffix</Label>
                    </div>
